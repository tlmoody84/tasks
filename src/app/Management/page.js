...
export default function ManagementPage() {
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    try {
      await updateDocument(db, "chores", editingChoreId, {
        type: editChoreType,
        details: editChoreDetails,
      });
    } catch (error) {
    ...
    const updatedChores = chores.map((chore) =>
      chore.id === editingChoreId
        ? { ...chore, type: editChoreType, details: editChoreDetails }
        : chore
    );
    ...
    <div className="container mx-auto px-6 py-9 bg-purple-700">
      ...
      <ul className="list-disc">
        {chores.map((chore) => (
          <li
            key={chore.id}
            className="flex justify-between items-center mb-6 text-pink-300"
          >
            <span
              className={`${
                chore.completed ? "text-black-500 line-through" : ""
              }`}
            >
              ...
              {editingChoreId === chore.id &&
                {
                  ...
                  <form onSubmit={handleSaveEdit}>
                    <div className="flex flex-col my-5 text-black">
                      <label htmlFor={`editChoreType-${chore.id}`}>
                        Chore:
                      </label>
                      <input
